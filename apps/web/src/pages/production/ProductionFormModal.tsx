...
export const ProductionFormModal: React.FC<ProductionFormModalProps> = ({
  ...
  const addWorkerRow = () => {
    const defaultEmp = workers.find((w) => !workerRows.some((r) => r.employeeId === w.id))?.id || workers[0]?.id || '';
    setWorkerRows((prev) => [
      ...prev,
      {
      ...
      <form id="production-form" onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div className="form-group" style={{ margin: 0 }}>
            ...
            <WheelDatePicker required value={date} onChange={(val) => setDate(val)} />
          </div>
            ...
            <div className="form-group animate-fade-in" style={{ gridColumn: 'span 2', margin: 0 }}>
              <label className="form-label">
                ...
                <span style={{ color: '#818cf8', fontWeight: 700 }}>
                  {t('auto.مرحلة_البند_المنفذة_المعدل_الق_731e85')}
                </span>
              </label>
              <select
                className="input-field"
              ...
              >
                {stages.map((stg) => (
                  <option key={stg.id} value={stg.id}>
                    {stg.name} {t('auto.وزن_25267a')}
                    {Math.round(Number(stg.percentage) * 100)}
                    {t('auto.إنتاجية_قياسية_12ab7e')}
                    {stg.standard_productivity})
                  </option>
                ))}
              </select>
            </div>
            ...
            >
              <div>
                ...
                <span style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>
                  {t('auto.مجموع_كميات_العمال_7827b9')}
                  {workerRows.reduce((a, b) => a + (Number(b.individualQuantity) || 0), 0)}
                  {t('auto.يجب_أن_يطابق_تمام_ا_الكمية_الف_4b790b')}
                  {actualQuantity})
                </span>
              </div>
