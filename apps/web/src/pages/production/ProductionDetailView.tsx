...
export const ProductionDetailView: React.FC<ProductionDetailViewProps> = ({
  ...
  const target = Number(record.targetQuantity) || 1;
    ...
    >
        ...
        >
          ...
          <div className="glass-card" style={{ padding: '0.85rem' }}>
            ...
            <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>{record.branchName}</div>
          </div>
          ...
          <div className="glass-card" style={{ padding: '0.85rem' }}>
            ...
            <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>
              {record.workAreaName || 'كامل الموقع'}
            </div>
          </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
            ...
            <div style={{ fontSize: '0.9rem' }}>
              ...
              <strong style={{ color: '#60a5fa', fontSize: '1.1rem' }}>{record.actualQuantity}</strong>
              <span style={{ color: 'var(--text-dim)' }}> / المستهدف: </span>
              <strong>{record.targetQuantity}</strong>
            </div>
          </div>
        ...
        {/* Workers Allocation Table (if individual) */}
        {record.workers && record.workers.length > 0 && (
          <div style={{ marginBottom: '1.5rem' }}>
            <h4 style={{ fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.6rem' }}>
              ...
              <span>توزيع حصص العمالة المشاركة (Rule R5 Breakdown)</span>
            </h4>
                ...
                <tbody>
                  {record.workers.map((w, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                      <td style={{ padding: '0.6rem 0.8rem', fontWeight: 600 }}>{w.employeeName || 'عامل'}</td>
                      <td style={{ padding: '0.6rem 0.8rem', fontFamily: 'monospace', color: 'var(--text-dim)' }}>
                        {w.employeeCode || '—'}
                      </td>
                      <td style={{ padding: '0.6rem 0.8rem' }}>{w.workerType}</td>
